...
cbind(sapply(alpha,function(x) quantile(abs(rend),x)),
      sapply(alpha,function(x) sum(abs(rend)>quantile(abs(rend),x))))
umbral<-1.5e-2
...
for(w in 1:length(windows)){
  for(i in 1:length(selected)){
    sigmas<-c()
    j<-selected[i]
    spec = getspec(garch.models[[j]])
    sigmas<-prediction.sigma(garch.models[[j]],val=T,w,t=length(val.indexes.vec[[w]]))
    garch.error[w,i,1]<-j
...
selected<-sort(c(23,14,13,21,7,8))
